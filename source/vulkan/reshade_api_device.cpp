...
bool reshade::vulkan::device_impl::map_resource_pitch(api::resource resource, uint32_t subresource, api::map_access, void **data, uint32_t *row_pitch)
{
	assert(resource.handle != 0);
	const resource_data &res_data = _resources.at(resource.handle);
	...
	if (res_data.allocation != nullptr)
	{
		...
		if (res_data.is_image())
		{
			...
			vk.GetImageSubresourceLayout(_orig, res_data.image, &img_subresource, &layout);
